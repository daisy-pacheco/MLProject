...
imputeData <- function(data){
  ...
  # meth[c(14, 18:27)] <- "" # personality vars to be ignored when included
...
addPayInteractions <- function(data){
...
interactions_train_data <- addPayInteractions(imputed_train_data)
interactions_test_data <- addPayInteractions(imputed_test_data)
