...
# load data
load("code/paper/data/final_train_data.RData")
load("code/paper/data/final_test_data.RData")
...
# LIME
lime_test <- final_test_data %>% 
  select(-id)
lime_test <- sapply(lime_test, as.numeric)

predict_model(x = lmerModel, newdata = lime_test, type = 'raw') %>%
  tibble::as_tibble()

# Train the explainer
explainer <- lime(final_train_data, 
                  lmerModel, 
                  bin_continuous = TRUE, 
                  quantile_bins = FALSE)

# Use the explainer on new observations
explanation <- lime::explain(final_test_data[1, ], explainer, n_features = 10)
