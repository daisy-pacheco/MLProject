...
#minimum MSE = 0.5323489
min(lasso_cv$cvm)  

#lambda for this minimum MSE = 0.0002411595
lasso_cv$lambda.min

#largest lambda value within one SE of the MSE = 0.009079502 (This shows how much we can constrain the coefficients while still maximizing predictive accuracy)
lasso_cv$lambda.1se
...
plot_coeff_evolution=function(regularization,type='Lasso')
{
  ...
  ggplot(coeff,aes(x=variable,y=value,color=name))+geom_line()+xlab(paste0(type,' regularisation'))+ylab('Value of coefficient')+scale_x_log10() + geom_vline(xintercept = lasso_cv$lambda.1se, linetype = "longdash") + geom_vline(xintercept = lasso_cv$lambda.min, linetype = "longdash")
}
